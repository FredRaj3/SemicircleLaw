...
\begin{definition}\label{def:semicirclePDFReal}
  ...
  \lean{semicirclePDFReal}
    The probability density function (p.d.f.) $f : \mathbb{R} \times \mathbb{R}_{\geq 0} \times \mathbb{R} \rightarrow \mathbb{R}$ 
   of the semicircle distribution is defined to be
   \[
    f(\mu,v,x) 
...
\begin{lemma}\label{lem:semicirclePDFReal_def}
  ...
  \uses{def:semicirclePDFReal}
    Given a mean $\mu \in \mathbb{R}$ and variance $v \in \mathbb{R}_{\geq 0}$, the p.d.f. $f : \mathbb{R} \rightarrow \mathbb{R}$ 
...
\begin{lemma}\label{lem:semicirclePDF_zero_var}
    ...
    \uses{def:semicirclePDFReal}
    If the variance $v$ is given to be $0$, then the p.d.f. of the semicircle distribution is the zero functional.
...
\end{lemma}
\begin{proof}
    By Definition \ref{def:semicirclePDFReal}, the squared root of a nonpositive number is defined to be zero.
    Hence, the probability density function with zero variance must be the zero functional.
\end{proof}
%---------%
%---------%
\begin{lemma}\label{lem:semicirclePDF_nonneg}
    ...
    \uses{def:semicirclePDFReal}
    The p.d.f. of a semicircle distribution is always nonnegative. 
\end{lemma}
\begin{proof}
   By Definition \ref{def:semicirclePDFReal}, the squared root of a nonpositive number is defined to be zero. 
   ...
   we conclude the probability density function is always nonnegative. 
\end{proof}
%---------%
%---------%
...
\end{lemma}
\begin{proof}
    In a space with second countable topology, a measurable function is always strongly measurable.
    % To be fixed!!!!!!!!!!!!!!!!!!!!!
\end{proof}
%---------%
%---------%
...
\begin{lemma}\label{lem:semicirclePDFReal_mul}
    ...
    of the semicircle distribution, the following relation is satisfied:
    \[
    f(\mu,v,c^{-1} x) = |c^{-1}| \cdot f(c^{-1} \mu,c^{-2} v,x)
    \]
    for any $u \in \mathbb{R}$, $v \in \mathbb{R}_{\geq 0}$, $x \in \mathbb{R}$, and nonzero $c \in \mathbb{R}$. 
\end{lemma}
\begin{proof}
   ...
   \[
   f(\mu,v,c^{-1} x)
   = \frac{1}{2πv} \sqrt{\bigl( 4v - ( c^{-1}x - μ)^2 \bigl)_+} 
   = \frac{1}{2πv} \sqrt{\bigl( 4v -  c^{-2}(x -  c^{-1}μ)^2 \bigl)_+} 
   = |c^{-1}| \frac{1}{2πv} \sqrt{\bigl( 4v \cdot c^{-2} - (x - (μ - y))^2 \bigl)_+}
