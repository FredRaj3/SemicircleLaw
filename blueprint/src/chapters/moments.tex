...
\begin{lemma}[Re-indexing the sum with counting argument : R-1-12 : lem:equation\_4.5\_2]
  ...
  \[
  \bE \Tr (\mathbf{Y}_\mathbf{i}^k) = \sum_{w \in \mathcal{G}_k} \Pi (w) \cdot \# \{ \mathbf{i} \in [n]^k : w_\mathbf{i} = w \}.
  \]
\end{lemma}
...
\begin{lemma}[Re-introducing the renormalization factor : R-1-13 : lem:equation\_4.5\_3]
  ...
  \[
  \frac{1}{n} \bE \Tr (\mathbf{X}_n^k) = \sum_{w \in \mathcal{G}_k} \Pi (w) \cdot \frac{n (n-1) \cdots (n - |G_w| + 1)}{n^{k/2+1}}.
  \]
\end{lemma}
...
\begin{lemma}[Simplifying the summation with the fact $\Pi (G,w) = 0$ in certain cases: R-1-16 : lem:equation\_4.8]
  ...
  = \sum_{w \in \mathcal{G}_{k, w \geq 2}} \Pi (w) \cdot \frac{n (n-1) \cdots (n - |G_w| + 1)}{n^{k/2+1}}.
  \]
...
\begin{proposition}
  ...
  $\lim_{n\to\infty}\bE\Tr(X_n^k) = \sum_{w\in\mathcal{G}^{k/2+1}_k} \Pi(G,w)$
\end{proposition}
...
\begin{lemma}
  ...
  $\Pi(w) = \prod_{e_c\in E^c} \bE(Y_{12}^{w(e_c)})$
\end{lemma}
