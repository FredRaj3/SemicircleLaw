...
\begin{lemma}[Trace of Expectation of Matrix]
    ...
    \notready
    \uses{lem:prod_expectation_matrix_multi_index, lem:matrix_powers_trace}
...
\begin{lemma}
  ...
  \uses{lem:lem_4.3,lem:equal_equiv_class_equal_expectation,lem:trace_expectation_of_matrix,lem:prod_expectation_matrix_multi_index}
  \[
...
\begin{lemma}
  ...
  \uses{def:g_k,def:matrix_multi_index,def:expectation_matrix_multi_index}
...
\begin{lemma}
  ...
  \uses{def:g_k_ge_2,lem:equation_4.5_3,lem:Pi.prod_eq_zero_if_w_le_two,lem:prod_expectation_matrix_multi_index}
  $$
...
\begin{lemma}
  ...
  \notready
  \uses{def:special_set_g, def:expectation_matrix_multi_index}
...
\begin{lemma}
  ...
  \notready
  \uses{lemma:special_g_edge_count}
  $t^{\#E} = t^{k/2}$
\end{lemma}
...
\begin{proposition}%[Equation 4.10 in \cite{Kemp2013RMTNotes}]
  ...
  \notready
  \uses{lemma:prod_expansion, lemma:w_2_case, lemma:def_t_rewrite, lemma:e_equals_k_over_two}
  %\uses{%%lemma:special_g_tree, %lemma:special_g_edge_count, prop:vertex_edge_tree_equality, prop:g_bound_self_edge, prop:g_bound_large_w, %def:expectation_matrix_multi_index} %uses: prop:matrix_moments_convergence for defn of t (but can replace)
  $\Pi(G,w) = t^{k/2}$.
\end{proposition}
