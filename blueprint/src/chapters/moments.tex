...
\begin{proof}
  \notready
  By construction, every edge in the joined graph $G_{{i}\#{j}}$ is traversed at least once by the union of the two paths $w_{i}$ and $w_{j}$.  Suppose that $e$ is an edge that is traversed only {\em once}.  This means that $w_{{i}\#{j}}(e) = 1$, and so it follows that the two values $w_{i}(e),w_{j}(e)$ are $\{0,1\}$.  Hence, the above expansion and the fact that $\E(Y_{11})=\E(Y_{12})=0$ show that $\pi(G_{{i}\#{j}},w_{i},w_{j}) = 0$ in this case.
\end{proof}


\begin{lemma}
  \label{lemma:simplied_var_trace_as_sum}
...
\begin{proposition}
  ...
  \uses{prop:vertex_edge_inequality, lemma:simpilified_var_trace_as_sum}
  \noteready
