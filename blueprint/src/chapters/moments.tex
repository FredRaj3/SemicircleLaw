...
\begin{lemma}[Matrix Multi Index and Graph Equivalence]
    ...
    \uses{def:matrix_multi_index,}
    Let $\mathbf{i} \in[n]^k$ be a $k$-index, $\mathbf{i}=\left(i_1, i_2, \ldots, i_k\right)$. Let
    $Y$ be a symmetric matrix. Then, we have the following:
     $$
...
\begin{lemma}[Graph Walk and Graph Count Equivalence]
    ...
    \uses{def:graph_walk_multi_index, def:graph_edge_count, def:matrix_multi_index, lem:multi_index_graph_equivalence}
    Let $\mathbf{i} \in[n]^k$ be a $k$-index, $\mathbf{i}=\left(i_1, i_2, \ldots, i_k\right)$.
...
\begin{definition}[Product of Expectation of Matrix Multi Index]
    ...
    \uses{def:matrix_multi_index, def:graph_edge_count, def:graph_self_edges, def:graph_connecting_edges, def:graph_walk_multi_index}
...
\begin{definition}[Length $|w|$ : R-1-3 : def:length\_of\_w]
  ...
  Given any graph $G=(V,E)$ and a walk $w$ on $G$, we let $|w|$ denote the length of $w$.
\end{definition}
%---------%
%---------%
...
%   \label{def:g_k_equiv}
%   \uses{def:graph_walk_multi_index,def:g_k,def:g_k_j}
...
\begin{lemma}[Re-introducing the renormalization factor : R-1-13 : lem:equation\_4.5\_3]
  ...
  \[
  \frac{1}{n} \bE \Tr (\mathbf{X}_n^k) = \sum_{(G,w) \in \mathcal{G}_k} \Pi (G,w) \cdot \frac{n (n-1) \cdots (n - |G_w| + 1)}{n^{k/2+1}}.
  \]
\end{lemma}
\begin{proof}
...
\begin{lemma}[Simplifying the summation with the fact $\Pi (G,w) = 0$ in certain cases: R-1-16 : lem:equation\_4.8]
  ...
  = \sum_{(G,w) \in \mathcal{G}_{k, w \geq 2}} \Pi (G,w) \cdot \frac{n (n-1) \cdots (n - |G_w| + 1)}{n^{k/2+1}}.
  \]
...
\begin{lemma}
  ...
  Suppose $k$ odd. Then, $\frac{n(n-1)\cdots(n-|G_w|+1)}{n^{k/2+1}} \le \frac{1}{\sqrt{n}}$.
\end{lemma}
...
\begin{proposition}%[Equation 4.9 in \cite{Kemp2013RMTNotes}]
  ...
  $\frac1n\bE\Tr(X_n^k) = \sum_{(G,w)\in\mathcal{G}^{k/2+1}_k} \Pi(G,w) \cdot \frac{n(n-1)\cdots(n-|G_w|+1)}{n^{k/2+1}} + O_k(n^{-1})$
