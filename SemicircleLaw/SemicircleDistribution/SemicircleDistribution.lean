...
lemma variance_fun_id_semicircleReal : Var[fun x ↦ x; semicircleReal μ v] = v := by
  ...
  -- We'll use the fact that the integral of $(x - \mu)^2$ with respect to the semicircle distribution is equal to $v$.
  have h_integral : ∫ x, (x - μ) ^ 2 * semicirclePDFReal μ v x = v := by
    -- We'll use the fact that the integral of $(x - \mu)^2$ over the semicircle distribution is $v$. This follows from the properties of the semicircle distribution.
    have h_integral : ∫ x in Set.Icc (μ - 2 * Real.sqrt v) (μ + 2 * Real.sqrt v), (x - μ)^2 * (Real.sqrt (4 * v - (x - μ)^2)) = (v : ℝ) * (2 * Real.pi * v) := by
      -- Let's simplify the integral.
      suffices h_integral_simplified : ∫ x in Set.Icc (-2 * Real.sqrt v) (2 * Real.sqrt v), x^2 * Real.sqrt (4 * v - x^2) = (v : ℝ) * (2 * Real.pi * v) by
        rw [ ← h_integral_simplified, ← MeasureTheory.integral_indicator, ← MeasureTheory.integral_indicator ] <;> norm_num [ Set.indicator ];
        rw [ ← MeasureTheory.integral_add_right_eq_self _ μ ] ; congr ; ext ; aesop;
        · exact Or.inr <| Real.sqrt_eq_zero_of_nonpos <| by contrapose! a; constructor <;> nlinarith [ Real.sqrt_nonneg v, Real.sq_sqrt <| show 0 ≤ ( v : ℝ ) by positivity ] ;
        · exact absurd ( h_1 ( by linarith ) ) ( by linarith );
      -- Let's simplify the integral using the substitution $x = 2\sqrt{v} \sin \theta$.
      suffices h_integral_subst : ∫ θ in Set.Icc (-Real.pi / 2) (Real.pi / 2), (2 * Real.sqrt v * Real.sin θ)^2 * Real.sqrt (4 * v - (2 * Real.sqrt v * Real.sin θ)^2) * (2 * Real.sqrt v * Real.cos θ) = (v : ℝ) * (2 * Real.pi * v) by
        rw [ ← h_integral_subst, MeasureTheory.integral_Icc_eq_integral_Ioc, ← intervalIntegral.integral_of_le ( by linarith [ Real.pi_pos, Real.sqrt_nonneg v ] ) ];
      ...
      suffices h_integral_simplified : ∫ θ in Set.Icc (-Real.pi / 2) (Real.pi / 2), 16 * v^2 * Real.sin θ^2 * Real.cos θ^2 = (v : ℝ) * (2 * Real.pi * v) by
        convert h_integral_simplified using 1;
        refine' MeasureTheory.setIntegral_congr_fun measurableSet_Icc fun x hx => _ ; ring ; norm_num [ Real.sin_sq, h ] ; ring;
        rw [ Real.sqrt_mul <| by positivity, Real.sqrt_mul <| by positivity ] ; ring;
        rw [ show ( Real.sqrt v ) ^ 4 = ( Real.sqrt v ^ 2 ) ^ 2 by ring, Real.sq_sqrt <| by positivity ] ; rw [ Real.sqrt_sq <| Real.cos_nonneg_of_mem_Icc ⟨ by linarith [ Real.pi_pos, hx.1 ], hx.2 ⟩ ] ; ring;
      rw [ MeasureTheory.integral_Icc_eq_integral_Ioc, ← intervalIntegral.integral_of_le ( by linarith [ Real.pi_pos ] ) ] ; norm_num [ mul_assoc, neg_div ] ; ring;
      norm_num [ mul_two ];
    -- Now use the fact that the integral of $(x - \mu)^2$ over the semicircle distribution is $v$ to conclude the proof.
    have h_final : ∫ x, (x - μ)^2 * (semicirclePDFReal μ v x) = (∫ x in Set.Icc (μ - 2 * Real.sqrt v) (μ + 2 * Real.sqrt v), (x - μ)^2 * (Real.sqrt (4 * v - (x - μ)^2))) / (2 * Real.pi * v) := by
      ...
      · contrapose! h_1;
        constructor <;> contrapose! h_1 <;> unfold ProbabilityTheory.semicirclePDFReal at * <;> aesop;
        · rw [ Real.sqrt_eq_zero_of_nonpos ( by nlinarith [ Real.sqrt_nonneg v, Real.mul_self_sqrt ( show 0 ≤ ( v : ℝ ) by positivity ) ] ) ];
        · rw [ Real.sqrt_eq_zero_of_nonpos ( by nlinarith [ Real.sqrt_nonneg v, Real.mul_self_sqrt ( show 0 ≤ ( v : ℝ ) by positivity ) ] ) ];
  ...
  · rw [ MeasureTheory.lintegral_withDensity_eq_lintegral_mul ];
    ...
    · fun_prop;
  · exact Filter.Eventually.of_forall fun x => mul_nonneg ( sq_nonneg _ ) ( ProbabilityTheory.semicirclePDFReal_nonneg _ _ _ );
  · exact Continuous.aestronglyMeasurable ( by exact Continuous.mul ( by continuity ) ( by exact ProbabilityTheory.Cont_semicirclePDFReal μ v ) )
...
lemma centralMoment_fun_odd_semicircleReal (μ : ℝ) (v : ℝ≥0) (n : ℕ) :
    ...
    · -- Use the substitution $u = x - \mu$ to transform the integral.
      have h_subst : ∫ x, (x - μ) ^ (2 * n + 1) * semicirclePDFReal μ v x = ∫ u, u ^ (2 * n + 1) * semicirclePDFReal μ v (u + μ) := by
